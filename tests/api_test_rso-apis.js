...
const testEditRSO = () => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const response = await axios.put(`http://localhost:3001/edit-rso/${test_rsoID}/1`, {
          rsoName: "Edited RSO",
...
const testDeleteRSO = () => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:3001/delete-rso/${test_rsoID}/1`);
...
async function testCheckLastAdminAPI(adminId) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
          ...
          const response = await axios.get(`http://localhost:3001/check-last-admin/${rsoId}/${adminId}`);   
...
const runTests = async () => {
    try {
      ...
      await testEditRSO();
      await sleep(500);
      await testCheckLastAdminAPI(2);
      await sleep(500);
      await testDisownRSO();
      await sleep(500);
      await testCheckLastAdminAPI(1);
      await sleep(500);
      await testDeleteRSO();
    } catch (error) {
      console.error("Error:", error.message);
